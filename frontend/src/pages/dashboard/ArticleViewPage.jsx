...
export const ArticleViewPage = () => {
  ...
  const seoLabel = seoScore >= 80 ? 'Excellent' : seoScore >= 60 ? 'Good' : 'Needs work';
  const keyTakeaways = Array.isArray(article?.keyTakeaways) ? article.keyTakeaways : null;
  const faqSchema = Array.isArray(article?.faqSchema) ? article.faqSchema : null;
  const suggestedTags = Array.isArray(article?.suggestedTags) ? article.suggestedTags : null;
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-6">
        ...
        <motion.article variants={fadeInUp} className="bg-white rounded-xl border border-[#F0F0F0] p-6 md:p-12 max-w-[780px] mx-auto lg:mx-0 font-inter">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm text-[#6B7280] mb-6">
            ...
            <span>·</span>
            <span data-testid="article-read-time">{readTime}</span>
          </div>

          {/* Suggested tags (new field) */}
          {suggestedTags && suggestedTags.length > 0 && (
            <div className="flex flex-wrap gap-1.5 mb-6" data-testid="article-suggested-tags">
              {suggestedTags.map((t) => (
                <span key={t} className="px-2 py-0.5 rounded-full bg-[#F0F0F0] text-[#6B7280] text-xs">
                  #{t.toString().replace(/^#/, '')}
                </span>
              ))}
            </div>
          ...
          {/* Key Takeaways — live data or fallback */}
          {keyTakeaways === null ? (
          ...
          {faqSchema && faqSchema.length > 0 && (
            <section className="mt-12 pt-8 border-t border-[#F0F0F0]" data-testid="article-faq">
              <h2 className="font-syne text-2xl font-bold text-[#0A0A0A] mb-6">Frequently Asked Questions</h2>
              <dl className="space-y-5">
                {faqSchema.map((f, i) => (
                  <div key={i} className="space-y-1">
                    <dt className="font-semibold text-[#0A0A0A]">{f.question}</dt>
                    <dd className="text-[15px] text-[#6B7280] leading-relaxed">{f.answer}</dd>
                  </div>
                ))}
              </dl>
            </section>
          ...
          <div className="lg:sticky lg:top-6 space-y-4">
            ...
            <div className="bg-white rounded-xl border border-[#F0F0F0] p-5">
              ...
              <div className="flex items-center gap-3 mb-4">
                <div className="w-16 h-16 rounded-full border-4 flex items-center justify-center" style={{ borderColor: seoColor }}>
                  <span className="font-syne text-xl font-bold text-[#0A0A0A]" data-testid="article-seo-score">{seoScore}</span>
                </div>
                <div>
                  <p className="text-2xl font-bold" style={{ color: seoColor }}>/100</p>
                  <p className="text-xs text-[#6B7280]">{seoLabel}</p>
                </div>
              </div>
              <ul>
                <SeoCheck label="Keyword usage"    status="good" />
                <SeoCheck label="Readability"      status="good" />
                <SeoCheck label="Meta description" status={metaDescription ? 'good' : 'missing'} value={metaDescription ? `${metaDescription.length} chars` : null} />
                <SeoCheck label="Title tag"        status={metaTitle ? 'good' : 'good'} value={metaTitle ? `${metaTitle.length} chars` : null} />
                <SeoCheck label="Internal links"   status="warn" />
                <SeoCheck label="Word count"       status="good" value={`${article?.wordCount || '2,100'} words`} />
              </ul>
              {(metaTitle || metaDescription) && (
                <div className="mt-3 pt-3 border-t border-[#F0F0F0] space-y-2 text-xs" data-testid="article-meta-preview">
                  {metaTitle && (
                    <div>
                      <p className="text-[#71717A] text-[10px] uppercase tracking-wide">Meta title</p>
                      <p className="text-[#0A0A0A] line-clamp-2">{metaTitle}</p>
                    </div>
                  )}
                  {metaDescription && (
                    <div>
                      <p className="text-[#71717A] text-[10px] uppercase tracking-wide">Meta description</p>
                      <p className="text-[#0A0A0A] line-clamp-3">{metaDescription}</p>
                    </div>
                  )}
                </div>
              )}
